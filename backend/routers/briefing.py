...
def get_today_briefing(db: Session = Depends(get_db)):
    ...
    for a in analyses:
        ...
        items.append(StockAnalysisItem(
            ...
            reason=a.reason,
            risk_level=None,       # raw_response에서 파싱 가능하나 단순화
            rsi_value=a.rsi_value,
        ))

    # 오늘 저장된 요약 텍스트는 raw_response 첫 번째 레코드에 보관
    summary = analyses[0].raw_response or "" if analyses else ""
...
def generate_briefing(db: Session = Depends(get_db)):
    ...
    for stock in stocks:
        ...
        analysis_rows.append(AiAnalysis(
            ...
            raw_response=None,  # 전체 요약은 아래에서 첫 번째 row에 저장
        ))
    ...
    # 첫 번째 row에 전체 요약 저장
    if analysis_rows:
        analysis_rows[0].raw_response = summary_text
